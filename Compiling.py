...
def auto_decode(data, depth=0, max_depth=20):
    ...
    for name, method in methods:
        try:
            ...
            elif name == "LZMA" and not (data_bytes.startswith(b'\xfd7zXZ') or data_bytes.startswith(b']\x00\x00')): should_try = False
            
            if should_try:
                result = method(data_bytes)
                if result is not None and result != data_bytes:
                    if isinstance(result, (bytes, str)):
                        decoded, path = auto_decode(result, depth + 1, max_depth)
                        return decoded, f"{name} -> {path}"
                    else:
    ...
    try:
        try:
            ...
            json_result = decode_json(data_str)
            return repr(json_result), "JSON"
        except: pass
        
        try:
            quopri_result = decode_quopri(data_bytes)
            decoded, path = auto_decode(quopri_result, depth + 1, max_depth)
            return decoded, f"Quoted-Printable -> {path}"
        except: pass

        try:
            data_str = data_bytes.decode('utf-8').strip()
            if len(data_str) > 2 and len(data_str) % 2 == 0 and all(c in '0123456789abcdefABCDEF' for c in data_str):
                hex_decoded = bytes.fromhex(data_str)
                decoded, path = auto_decode(hex_decoded, depth + 1, max_depth)
                return decoded, f"Hex -> {path}"
        except: pass
            
    except Exception: pass
    
    try:
        return data_bytes.decode('utf-8'), "Text"
    except UnicodeDecodeError:
        return f"Binary data (hex): {data_bytes.hex()}", "Binary"
...
def main():
    ...
    while True:
        display_menu()
        choice = input(f"\n{Fore.CYAN}🎯 Enter your choice {Fore.WHITE}(1-16, 99 to exit): {Fore.GREEN}").strip()
        ...
        if choice in [str(i) for i in range(1, 17)]: 
            filename = get_file_input_with_selector()
            if not filename:
                continue
            
            display_loading(f"Reading file: {filename}")
            ...
            if choice == '16':
                try:
                    ...
                    display_error("File is binary or not valid UTF-8. Cannot compile to .pyc.")
                    continue
                
                print(f"\n{Fore.CYAN}🐍 Select Python Bytecode Version for Compilation (e.g., 3.8, 3.11):")
                py_version = input(f"{Fore.WHITE}➤ {Fore.GREEN}").strip()
                if py_version not in MAGIC_NUMBERS:
                    display_warning(f"Version {py_version} not supported or invalid. Defaulting to 3.8.")
                    py_version = '3.8'
                output_ext = ".pyc"
                
            try:
                if choice == '1':
                    display_loading("Decoding Base64"); result = decode_base64(data); encoding_type = "Base64"
                elif choice == '2':
                    display_loading("Decoding Base32"); result = decode_base32(data); encoding_type = "Base32"
                elif choice == '3':
                    display_loading("Decoding Base16/Hex"); result = decode_base16(data); encoding_type = "Base16/Hex"
                elif choice == '4':
                    display_loading("Decoding Marshal"); result = decode_marshal(data); encoding_type = "Marshal"
                elif choice == '5':
                    display_loading("Decoding Zlib"); result = decode_zlib(data); encoding_type = "Zlib"
                elif choice == '6':
                    display_loading("Decoding Gzip"); result = decode_gzip(data); encoding_type = "Gzip"
                elif choice == '7':
                    display_loading("Decoding Lzma"); result = decode_lzma(data); encoding_type = "Lzma"
                elif choice == '8':
                    display_loading("Decoding BZ2"); result = decode_bz2(data); encoding_type = "BZ2"
                elif choice == '9':
                    display_loading("Decoding Pickle"); result = decode_pickle(data); encoding_type = "Pickle"
                elif choice == '10':
                    display_loading("Decoding UU (Note: Standard UU library is tricky)"); result = decode_uu(data); encoding_type = "UU"
                elif choice == '11':
                    display_loading("Decoding Quoted-Printable"); result = decode_quopri(data); encoding_type = "Quoted-Printable"
                elif choice == '12':
                    display_loading("Decoding JSON"); result = decode_json(data); encoding_type = "JSON"
                elif choice == '13':
                    display_loading("Auto detecting layers"); result, encoding_type = auto_decode(data)
                elif choice == '14':
                    display_loading("Decompiling Python bytecode"); result = process_python_bytecode(data); encoding_type = "Pyc Decompilation"
                elif choice == '15':
                    result, encoding_type = decode_advanced_obfuscation(data)
                    if result is None: display_error("Could not decode advanced obfuscation"); continue
                
                elif choice == '16':
                    display_loading(f"Compiling Python Source to .pyc (Python {py_version})")
                    result = compile_to_pyc(source_code, py_version)
                    encoding_type = f"Source to Pyc (Python {py_version})"

                # Display result - using static color
                if result is None:
                    display_error("Decoding/Compilation failed or no result")
                    continue
                
                print(f"\n{Fore.GREEN}✅ {encoding_type} successful!")
                print(f"{Fore.CYAN}╔════════════════════════════ RESULT ════════════════════════════╗")
                
                if isinstance(result, bytes):
                    try:
                        result_str = result.decode('utf-8')
                        print(f"{Fore.WHITE}{result_str}")
                    except UnicodeDecodeError:
                        print(f"{Fore.WHITE}Binary data (size: {len(result)} bytes). Hex snippet: {result.hex()[:100]}...")
                elif isinstance(result, str):
                    print(f"{Fore.WHITE}{result}")
                else:
                    print(f"{Fore.WHITE}{repr(result)}")
                
                print(f"{Fore.CYAN}╚══════════════════════════════════════════════════════════╝")
                
                save_choice = input(f"\n{Fore.CYAN}💾 Save result? {Fore.WHITE}(y/n): {Fore.GREEN}").strip().lower()
                if save_choice == 'y':
                    output_file = get_output_filename(default_extension=output_ext)
                    if output_file:
                        save_result(result, output_file)
            
            except Exception as e:
                display_error(f"An unexpected error occurred during processing: {e}")
        
        else:
            display_error("Invalid choice. Please try again.")

        continue_choice = input(f"\n{Fore.CYAN}🔄 Continue with another file? {Fore.WHITE}(y/n): {Fore.GREEN}").strip().lower()
...
if __name__ == "__main__":
    ...
    try:
        subprocess.run(["pycdc", "-h"], capture_output=True, check=False, timeout=5) 
    except FileNotFoundError:
        print(f"\n{Fore.RED}!!! WARNING: 'pycdc' not found. !!!")
        print(f"{Fore.RED}Decompilation (options 14 & 15) will fail.")
        print(f"{Fore.RED}To fix: Install pycdc (check online guides).")
    except subprocess.TimeoutExpired:
        pass
    except Exception:
        pass
